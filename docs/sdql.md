...
- `{"operator": "REGEX_MATCH", "column": <column>, "expression": <regular expression in standard format >}`.  _Columns of type *STRING* only_. Returns the list of rows where the entry in column `<column>` matches the regular expression `<expression>`.
